...
describe("Circuit test", function () {
    ...
    it("Circuit test on leftmost path", async () => {
        ...
        let currentIndex = 1;
        const pathElements = [hashes[currentIndex]];
...
describe("OTP Factory Contract", function () {
    ...
    beforeEach(async function () {
        ...
        const Tx = await factory.createOTP(verifier.address, root);
        const tx = await Tx.wait();

        const address = tx.events[0].args.newAddress;

        const OTP = await ethers.getContractFactory("OTP");
